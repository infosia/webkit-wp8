...
/*
General Idea:
Logger = require("loggerlua")
mylog = Logger.Create()
mylog:LogEvent(5, "keyword", "sub", "hello", "world", "moon")

Output:
2012-10-04_18:36:37.961    keyword:    sub:    PRI=5
hello	world	moon
^@END:$
	...
	if(logger->fileHandle != NULL)
	{
		...
		for (i=start_text_index; i<=n; i++)
		{
			...
			if (i>start_text_index)
			{
				...
				{
					ret_val = ((LoggerOpaqueData*)logger->opaqueLoggerData)->customPuts(logger, ((LoggerOpaqueData*)logger->opaqueLoggerData)->customCallbackUserData, "\t");
				}
				if(ret_val >= 0)
				{
			...
			{
				ret_val = ((LoggerOpaqueData*)logger->opaqueLoggerData)->customPuts(logger, ((LoggerOpaqueData*)logger->opaqueLoggerData)->customCallbackUserData, s);
			}
			if(ret_val >= 0)
			{
